...
def export_results(dataframe, kind, fname, storage_client, bucket_name, prefix_path):
    ...
    cloud_csv_filename = f"{prefix_path}/{current_timestamp}-{fname}.csv"
    cloud_xlsx_filename = f"{prefix_path}/{current_timestamp}-{fname}.xlsx"
...
def prepare_bigquery_data(df, storage_client, bucket_name, prefix_path, fname, current_timestamp, dataset_name, table_name, project_id):
    ...
    timestamp = datetime.now().isoformat()  # Convert datetime to ISO 8601 string format (Time equals utc timezone)
...
def start_script(request):
    ...
    prefix_path = data.get('prefix_path')
    ...
    if project_id == "your-project-id" and bucket_name == "your-bucket":
        ...
        print(f"Used default project_id: {project_id}")
        print(f"Used default bucket_name: {bucket_name}")

    storage_client = storage.Client(project=project_id)  
    start_time = time.time()
    print("Starting script:")
    print("Scraping web. This can take a while...")
    fname_events = "dbz_db_story_events"
    fname_characters = "dbz_db_characters"
    ...
    timestamp_events = export_results(story_events_df, "story events", fname_events, storage_client, bucket_name, prefix_path)  
    timestamp_characters = export_results(characters_df, "characters", fname_characters, storage_client, bucket_name, prefix_path)
    ...
    # Publish a new message to the bew Pub/Sub topic
